      $(function() {
      $("input[id='cmpBtn']").click(function(event) {
        var A = false;
        var F = false;
        var S = false;
        ...
        checkboxes.forEach((checkbox) => {
                 if (checkbox.value=="A" && checkbox.checked)
                  checkbox.value = true;
                 if (checkbox.value=="F" && checkbox.checked)
                  checkbox.value = true;
                 if (checkbox.value=="S" && checkbox.checked)
                  checkbox.value = true;  });
        var packages = document.getElementsByName("radiopid");
        packages.forEach((package)=>{
          if (package.checked)
                document.getElementById("package_selected").value = package.value;
        });
        $.ajax({
          cache:false,
          url:$SCRIPT_ROOT+"/advancedmseview/advcalc/",
          type:"PUT",
          dataType: "json",
                contentType:"application/json",
                data: JSON.stringify({"fMin": fMin, "fMax": fMax, "fLevel":fLevel,"aMin": fMin, "aMax": fMax,"aLevel":fLevel,"sMin": fMin, "sMax": fMax,"sLevel":fLevel,"pk":pk, "A":A,"F":F,"S":S }),       
...
$(function() {
  ...
  $("input[name=radiopid]").on('change', function(event) {
    ...
    .done(function(data) {
      $('#fCur').val(data.bio_f_percent||0);
      $('#fMin').val(data.bio_f_percent/2||0);
      $('#fMax').val(2*data.bio_f_percent||0.05);
      $('#fLevel').val(5);
      $('#sCur').val(data.bio_catch_mt||0);
      $('#sMin').val(data.bio_catch_mt/2||0);
      $('#sMax').val(2*data.bio_catch_mt||10000000000000);
      $('#sLevel').val(5);
      $('#aCur').val(data.sec_recreational||0);
      $('#aMin').val(3*data.sec_recreational/4||0);
      $('#aMax').val(3*data.sec_recreational/2||50);
      $('#aLevel').val(5);
    });
